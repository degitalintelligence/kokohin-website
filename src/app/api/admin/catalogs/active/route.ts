...
export async function POST(request: Request) {
  ...
  if (isActive === null) return errorResponse('BAD_REQUEST', 'Missing is_active', 400)
  ...
  if (error) {
    return errorResponse('INTERNAL_ERROR', 'Failed to update active flag', 500, error.message)
  }
  if (!updated) {
